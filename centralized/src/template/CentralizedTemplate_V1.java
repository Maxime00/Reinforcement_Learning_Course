...
public class CentralizedTemplate_V1 implements CentralizedBehavior {
...
//		System.out.println("Agent " + agent.id() + " has tasks " + tasks);
        ...
        List<Strategy> neighbours = new ArrayList<Strategy>();
    ...
    private Strategy selectInitialStrategy() {
    	...
    	//need to verify if total number of tasks exceed sum of capacities
    	
		initialStrategy.nextAction[numActions] = new Action(0, PICKUP, numVehicles);
		
    	for(int i = numActions+1; i < numActions + numVehicles; i++)
    		initialStrategy.nextAction[i] = null;
    	
    	for(int i = 0; i < numTasks; i++) {
    		initialStrategy.nextAction[i] = new Action(i, DELIVER, numTasks);
    		if(i+1 != numTasks)
    			initialStrategy.nextAction[numTasks + i] = new Action(i+1, PICKUP, numTasks);
    		else 
    			initialStrategy.nextAction[numTasks + i] = null;

    		initialStrategy.time[i] = j;
    		initialStrategy.time[numTasks + i] = j+1;
    		j = j+2;
    		initialStrategy.vehicle[i] = 0;
    		//System.out.println("test-initialStrat "+initialStrategy.nextAction[numTasks + i].task+" complete");
    	}
		for(int t = 0; t < numActions; t++) {
			if(t % 2 == 0)
				initialStrategy.load[0][t] = tasks.get(t % numTasks).weight;
			else
				initialStrategy.load[0][t] = 0;
		}
    ...
    private List<Strategy> findNeighbors(Strategy oldStrategy){
    	...
    	/*for(Strategy strategy:neighbors) {
    		f++;
    		strategyToPlans(strategy);
    	}*/
    ...
    private Strategy changeVehicle(Strategy strategy, int vehicle1, int vehicle2) {
    	...
    	//System.out.println("test-changeVehicle variables inti complete");
		...
		//System.out.println("test-changeVehicle vehicle2 change complete");
		updateTime(newStrategy, vehicle1);
		updateTime(newStrategy, vehicle2);
	//	System.out.println("test-changeVehicle updateTime complete");
		for(int i = 0; i< numVehicles; i++) {
			for(int j = 0; j < numActions; j++)
				newStrategy.load[i][j] = 0;
    	}
    			...
    			do {
		    		...
		    		if(nextAction == null) {
			    		...
			    		if(strategy.load[v][strategy.time[nextAction.indexer + nextAction.task]] > vehicles.get(v).capacity()) {
			    			...
			    			//System.out.println("test-verifyConstraints ERROR : load = "+strategy.load[v][strategy.time[nextAction.indexer + nextAction.task]]+"");
			    			return false;
			    		}
...
private Strategy localChoice(List<Strategy> neighbors) {
    	...
    	//enlever la oldStrategy de neighbours?
    	neighbors.remove(0);
    	...
    	for(Strategy neighbor : neighbors) {
    		...
    		for(int i = 0; i < numActions; i++) {
    			if(neighbor.nextAction[i] != null) {
	    			...
	    			if(i < numTasks && neighbor.nextAction[i].command == DELIVER ) {
	...
	private List<Plan> strategyToPlans(Strategy strategy){
		...
		//i = vehicle number
		for( int i = 0 ; i < numVehicles ; i++) {
			Plan plan = new Plan(vehicles.get(i).homeCity());
			
			Action action1 = strategy.nextAction[numActions + i];
			...
			if(action1 != null) {
				...
				Action action2 = strategy.nextAction[action1.indexer + action1.task];
	...
	//	System.out.println("counterActions = "+counter+"" );
		return plans;
	}
